...
\title{\pkg{mvtnorm}: New Numerical Algorithm for Multivariate Normal Probabilities}
\author{Xuefei Mi, Tetsuhisa Miwa and Torsten Hothorn}

\maketitle

\cite{Miwa2003} proposed a numerical algorithm for
...
& = & \int_0^\infty \dots \int_0^\infty \phi_m(\x; \mu, \Cor) \, dx_1 \dots dx_m
\end{eqnarray*}
where $\mu =(\mu_1,...,\mu_m)^\top$ is the mean and
and $\Cor = (\rho_{ij})$ the correlation matrix of $m$ multivariate
normal distributed random variables $X_1, \dots, X_m \sim 
\mathcal{N}_m(\mu, \Cor)$. The function $\phi_m$ denotes the 
...
F_m(\c) & = & \Prob \{X_i \leq c_i; \, 1\leq i\leq m \} \\
        ...
        & = & P_m(-\mu+\c,\Cor).
\end{eqnarray*}
The $m$-dimensional non-centered orthant one-sided probability can be calculated from at most
$(m-1)!$ non-centered probabilities with positive definite
tri-diagonal correlation matrix. The algorithm for calculating such
...
that $p = 0.6536804$ with given correlation matrix $\Cor$. 
...
special structure. E.g., when $\Cor$ is a $m$-dimensional tri-diagonal
...
\end{eqnarray*}
the value of $P_m(\mathbf{0}, \Cor)$ is
$((1+m)!)^{-1}$ \citep{Miwa2003}. The accuracy of Miwa' algorithm
($G = 128$ grid points) and the Genz \& Bretz algorithm
%%TH: what exactly is the tolerance??? abseps or releps?
(with absolute error tolerance $\varepsilon=10^{-4},10^{-5},10^{-6}$) for probabilities with
...
tables. When the dimension is
larger than five, Genz \& Bretz' algorithm with error tolerance smaller than
$10^{-5}$ is hard to achieve.
...
consumption of Miwa's algorithm and Genz \& Bretz' algorithm for this
situation are compared in Table \ref{table2}.  

\subsection{Time consumption}
Miwa's algorithm is a numerical method which has advantages in achieving higher accuracy with less time consumption 
compared to the stochastic method.
However, Miwa's algorithm has some
...
\hline
\end{tabular}
\caption{Time consumption of non-centered orthant
...
to quasi-randomized Monte-Carlo procedures which are computed by default. 
